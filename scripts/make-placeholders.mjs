...
function placeholderSvg({ w, h, label, fig }) {
  ...
  for (let x = 0; x <= w; x += 80) {
    grid.push(`<line x1="${x}" y1="0" x2="${x}" y2="${h}" stroke="#e2ddce" stroke-width="1"/>`);
  }
  for (let y = 0; y <= h; y += 80) {
    grid.push(`<line x1="0" y1="${y}" x2="${w}" y2="${y}" stroke="#e2ddce" stroke-width="1"/>`);
  }
  const cx = w / 2;
  const cy = h / 2;
  // Palette tracks src/styles/tokens.css: cream paper, navy ink, faded rules.
  return Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}">
  <rect width="${w}" height="${h}" fill="#f2eee3"/>
  ${grid.join("")}
  <rect x="24" y="24" width="${w - 48}" height="${h - 48}" fill="none" stroke="#d6d0bf" stroke-width="2"/>
  <line x1="24" y1="24" x2="${w - 24}" y2="${h - 24}" stroke="#d6d0bf" stroke-width="1"/>
  <line x1="${w - 24}" y1="24" x2="24" y2="${h - 24}" stroke="#d6d0bf" stroke-width="1"/>
  <rect x="${cx - 150}" y="${cy - 46}" width="300" height="92" fill="#faf8f1" stroke="#b5ae99" stroke-width="1"/>
  <text x="${cx}" y="${cy - 8}" font-family="monospace" font-size="26" letter-spacing="4" fill="#16386f" text-anchor="middle">${label}</text>
  <text x="${cx}" y="${cy + 26}" font-family="monospace" font-size="16" letter-spacing="3" fill="#5d6a8a" text-anchor="middle">${fig} · IMAGE PENDING</text>
</svg>`);
}
